    ...
    reportsTableBody?.addEventListener('click', async (e) => {
        ...
        if (button) {
            ...
            try {
                ...
                reportContent += `*Total Frais Livraison :* ${formatAmount(reportDetails.total_delivery_fees)}\n`;
                
                if (reportDetails.total_packaging_fees > 0) {
                    reportContent += `*Total Frais Emballage :* ${formatAmount(reportDetails.total_packaging_fees)}\n`;
                }
                if (reportDetails.total_storage_fees > 0) {
                    reportContent += `*Total Frais Stockage (jour) :* ${formatAmount(reportDetails.total_storage_fees)}\n`;
                }
                if (reportDetails.total_expedition_fees > 0) {
                    reportContent += `*Total Frais Expédition :* ${formatAmount(reportDetails.total_expedition_fees)}\n`;
                }
