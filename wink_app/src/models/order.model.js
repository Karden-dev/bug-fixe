        ...
        try {
            ...
            const ordersQuery = `
                SELECT
                    ...
                    (SELECT SUM(amount) FROM debts WHERE order_id = o.id AND type = 'expedition' AND status = 'pending' AND DATE(created_at) = ?) as expedition_fee
                FROM orders o
                LEFT JOIN order_items oi ON o.id = oi.order_id
                WHERE o.shop_id = ? AND DATE(o.created_at) = ?
                      AND o.status IN ('delivered', 'failed_delivery')
                GROUP BY o.id
                ORDER BY o.created_at ASC;
            `;
            const [orders] = await connection.execute(ordersQuery, [date, shopId, date]);
