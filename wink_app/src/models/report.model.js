        ...
        try {
            ...
            const query = `
                SELECT
                    ...
                    COALESCE(todays_orders.total_expedition_fees, 0) AS total_expedition_fees,
                ...
                LEFT JOIN (
                    ...
                    SELECT shop_id, SUM(CASE WHEN type = 'storage' THEN amount ELSE 0 END) AS storage_fee_today
            ...
            return rows.map(row => {
                ...
                const merchantDebts = parseFloat(row.total_delivery_fees) + parseFloat(row.total_expedition_fees) + parseFloat(row.total_packaging_fees) + parseFloat(row.total_storage_fees) + parseFloat(row.previous_debts);
                ...
                return {
                    ...
                    total_expedition_fees: parseFloat(row.total_expedition_fees || 0),
