...
router.get('/pending-preparation', verifyToken, isAdmin, async (req, res) => {
    try {
        ...
        // Filtre côté serveur pour enlever celles qui sont déjà "en route" ou plus loin, si le modèle ne le fait pas strictement.
        // Le modèle findOrdersToPrepare filtre sur status IN ('in_progress', 'ready_for_pickup').
        // Si le livreur a confirmé la récupération (picked_up_by_rider_at est set), on doit la retirer de la liste de préparation/vérification
        const filteredOrders = ordersToPrepare.filter(order => !order.picked_up_by_rider_at);
...
router.put('/:id/status', verifyToken, async (req, res) => { 
    try {
        const { id } = req.params;
        const { status, amount_received, payment_status } = req.body;
