...
const findConversationsForAdmin = async (adminUserId, filters) => {
    ...
    try {
        ...
        // CORRECTION : Remplacement de LEFT JOIN LATERAL par des sous-requêtes corrélées simples
        let query = `
            SELECT
                o.id as order_id,
                o.customer_phone,
                ...
                o.is_urgent,
                o.is_archived,
                -- Sous-requête 1 : Récupérer le contenu du dernier message
                (
                ...
                -- Sous-requête 2 : Récupérer l'heure du dernier message
                (
                    SELECT created_at
                    FROM order_messages
                    WHERE order_id = o.id
                ...
                -- Sous-requête 3 : Compteur de messages non lus
                (SELECT COUNT(*)
        ...
        // Le tri final est géré côté JS pour prioriser 'is_urgent' et 'last_message_time'
        // Pas de clause ORDER BY ici pour ne pas interférer avec les sous-requêtes
