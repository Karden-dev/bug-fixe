...
const getCashCollected = async (startDate, endDate) => {
    const connection = await db.getConnection();
    try {
        // Le filtre de date porte sur 'validated_at' car c'est la date de l'entrée en caisse réelle.
        const dateFilter = getDateFilter(startDate, endDate, 'validated_at'); 
...
const getExpensesAndWithdrawals = async (startDate, endDate) => {
    ...
    try {
        ...
        const [result] = await connection.execute(
            `
            SELECT 
                -- Retourne la valeur absolue (en changeant le signe)
...
/**
 * Calcule les totaux de Créances remboursées et non remboursées.
 * IMPORTANT: Les créances NON remboursées sont filtrées par created_at (flux de création sur la période).
 * @param {string} startDate
 * @param {string} endDate
...
const getDebtMetrics = async (startDate, endDate) => {
    const connection = await db.getConnection();
    try {
        // 1. Créances Remboursées (status='paid') - Filtre par settled_at (flux de trésorerie)
        ...
        // 2. Créances Non Remboursées (status='pending') - Filtre par created_at (flux de création)
        ...
        const pendingDebts = parseFloat(pendingResult[0].total_pending);
 ...
 * IMPORTANT: Les manquants NON remboursés sont filtrés par created_at (flux de création sur la période).
 * @param {string} startDate
 * @param {string} endDate
...
const getShortfallMetrics = async (startDate, endDate) => {
    const connection = await db.getConnection();
    try {
        // 1. Manquants Remboursés (status='paid') - Filtre par settled_at (flux de trésorerie)
        const settledFilter = getDateFilter(startDate, endDate, 'settled_at');
        const [paidResult] = await connection.execute(
            `
            SELECT 
        ...
        // 2. Manquants Non Remboursés (status='pending'/'partially_paid') - Filtre par created_at
        const createdFilter = getDateFilter(startDate, endDate, 'created_at');
        const [pendingResult] = await connection.execute(
            `
            SELECT 
