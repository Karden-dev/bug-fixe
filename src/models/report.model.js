    ...
    findReportsByDate: async (date) => {
        ...
        try {
            ...
            const query = `
                ...
                LEFT JOIN (
                    SELECT shop_id, SUM(amount) AS storage_fee_today
                    FROM debts WHERE DATE(created_at) = ? AND type = 'storage' -- FIX: Restreint uniquement aux frais de stockage
                    GROUP BY shop_id
                ) AS todays_debts ON s.id = todays_debts.shop_id
                LEFT JOIN (
                    SELECT shop_id, SUM(amount) AS total_pending_debts
                    FROM debts WHERE status = 'pending' AND DATE(created_at) < ? AND type NOT IN ('daily_balance', 'other', '') GROUP BY shop_id
