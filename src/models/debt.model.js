...
const findAll = async (filters) => {
    ...
    try {
        ...
        // NOUVEAU : Filtre d'exclusion de type
        if (filters.excludeType && Array.isArray(filters.excludeType)) {
            const placeholders = filters.excludeType.map(() => '?').join(', ');
            query += ` AND d.type NOT IN (${placeholders})`;
...
const create = async (debtData) => {
    const connection = await dbConnection.getConnection();
    try {
        // Ajout de la date de création personnalisée
        const created_at = debtData.created_at ? moment(debtData.created_at).format('YYYY-MM-DD HH:mm:ss') : moment().format('YYYY-MM-DD HH:mm:ss');
        
        const query = 'INSERT INTO debts (shop_id, amount, type, comment, status, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)';
        const [result] = await connection.execute(query, [debtData.shop_id, debtData.amount, debtData.type, debtData.comment, 'pending', debtData.created_by, created_at]);
