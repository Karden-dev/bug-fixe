...
module.exports = {
    ...
    update: async (orderId, orderData, userId) => {
        ...
        try {
            ...
            if (oldOrder.status !== 'pending' && !['in_progress', 'ready_for_pickup', 'en_route', 'return_declared'].includes(oldOrder.status)) { // Inclure les statuts intermédiaires
            ...
            if (newOrder.status !== 'pending' && !['in_progress', 'ready_for_pickup', 'en_route', 'return_declared'].includes(newOrder.status)) {
    ...
    updateStatus: async (orderId, newStatus, amountReceived = null, newPaymentStatus = null, userId) => {
        ...
        try {
            ...
            const intermediateStatuses = ['pending', 'in_progress', 'reported', 'ready_for_pickup', 'en_route', 'return_declared'];
            ...
            if (newStatus === 'delivered') {
                ...
                finalPaymentStatus = (finalAmountReceived !== null && finalAmountReceived > 0) ? 'cash' : 'pending';
                 ...
                 // Si on réinitialise à 'en_route' ou 'reported'
    ...
    remove: async (orderId) => {
        ...
        try {
            ...
            // Annuler l'impact financier sur le bilan (si applicable)
            const intermediateStatuses = ['pending', 'in_progress', 'reported', 'ready_for_pickup', 'en_route', 'return_declared'];
    ...
    findAll: async (filters) => {
        const connection = await dbConnection.getConnection();
        // Base de la requête avec jointures pour récupérer les noms
        ...
        try {
            ...
            // Ajouter les filtres de date s'ils sont fournis
            ...
            } else if (!filters.status) { // Si aucun filtre de statut n'est explicitement fourni (comportement par défaut)
                // Exclure les statuts 'return_declared' et 'returned'
    ...
    markAsReadyForPickup: async (orderId, preparedByUserId) => {
        ...
        try {
            ...
            // --- AJOUT : Envoyer la notification WebSocket ---
            try {
                ...
                if (orderDetails.length > 0 && orderDetails[0].deliveryman_id) {
                    ...
                    // Utiliser sendNotification du service WebSocket
    ...
    confirmPickupByRider: async (orderId, riderUserId) => {
        ...
        try {
            ...
            // --- AJOUT : Notifier l'admin via WebSocket ---
            try {
                 const orderDetails = await module.exports.findById(orderId); // Récupérer les détails
                 if (orderDetails) {
                     // Envoyer à l'admin (supposons ID 1, ou récupérer les admins actifs)
                     const adminUserId = 1; // À adapter si plusieurs admins
                     webSocketService.sendNotification(adminUserId, 'ORDER_PICKED_UP_BY_RIDER', {
                         order_id: orderId,
                         rider_name: orderDetails.deliveryman_name || 'Livreur inconnu'
                     });
                     console.log(`Notification ORDER_PICKED_UP_BY_RIDER envoyée à l'admin pour CDE ${orderId}`);
                 }
             } catch (notifyError) {
    ...
    declareReturn: async (orderId, riderUserId, comment = null) => {
        ...
        try {
            ...
            // -- CORRECTION : Autoriser le retour même si 'reported' --
            if (!['en_route', 'failed_delivery', 'cancelled', 'reported'].includes(currentStatus)) {
            ...
            try {
                 const orderDetails = await module.exports.findById(orderId); // Récupérer les détails
                 if (orderDetails) {
                     const adminUserId = 1; // À adapter
                     webSocketService.sendNotification(adminUserId, 'NEW_RETURN_DECLARED', {
                         order_id: orderId,
                         tracking_id: trackingId, // Envoyer aussi l'ID du suivi
                         rider_name: orderDetails.deliveryman_name || 'Livreur inconnu',
                         shop_name: orderDetails.shop_name || 'Marchand inconnu'
                     });
                     console.log(`Notification NEW_RETURN_DECLARED envoyée à l'admin pour CDE ${orderId}`);
                 }
             } catch (notifyError) {
    ...
    confirmHubReception: async (trackingId, adminUserId) => {
        ...
        try {
            ...
            // L'appel à `updateStatus` gérera aussi l'archivage et l'historique
