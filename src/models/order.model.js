...
module.exports = {
    ...
    /**
     * (NOUVELLE FONCTION - CORRECTION DE BUG V2)
     * Récupère les commandes livrées le jour même (il y a au moins 3 heures)
     * n'ayant pas encore reçu de demande d'avis.
     * Cible: Statut 'delivered', ai_review_sent = 0, 
     * updated_at est AUJOURD'HUI, 
     * updated_at est plus vieux que 3 heures.
     * (V2: Inclut les détails de la commande pour personnalisation)
     */
    getDeliveredOrdersPendingReview: async (db) => {
        const connection = await db.getConnection();
        try {
            // Assurez-vous que la colonne 'ai_review_sent' existe.
            // ALTER TABLE orders ADD COLUMN ai_review_sent TINYINT(1) DEFAULT 0;
            ...
            const query = `
                SELECT 
                    ...
                    (SELECT item_name FROM order_items WHERE order_id = o.id LIMIT 1) as first_item_name
                FROM orders o
                LEFT JOIN users u ON o.deliveryman_id = u.id
                WHERE 
                    o.status = 'delivered'
                    AND o.ai_review_sent = 0
