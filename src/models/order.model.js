...
module.exports = {
     ...
     * Interdit la modification si le colis a déjà été récupéré par le livreur.
     */
    update: async (orderId, orderData, userId) => {
        ...
        try {
            ...
            if (oldOrder.picked_up_by_rider_at) {
                await connection.rollback(); // Annuler avant de lancer l'erreur
                throw new Error("Modification non autorisée : Le colis a déjà été récupéré par le livreur.");
            }
            // --- FIN VÉRIFICATION ---
    ...
    remove: async (orderId) => {
        ...
        try {
            ...
            await connection.execute('DELETE FROM returned_stock_tracking WHERE order_id = ?', [orderId]); // <-- ASSURE LA DISPARITION DE preparation.html
    ...
    declareReturn: async (orderId, riderUserId, comment = null) => {
        ...
        try {
            ...
            const [existingReturn] = await connection.execute(
                ...
                [orderId, 'pending_return_to_hub', 'received_at_hub'] // Statuts indiquant un retour actif/terminé
            );
            ...
            await connection.commit(); // Valider
            return { success: true, trackingId: trackingResult.insertId }; // Renvoyer l'ID du suivi créé
        } catch (error) {
            await connection.rollback(); // Annuler
    ...
    confirmHubReception: async (trackingId, adminUserId) => {
        ...
        try {
            ...
            const [trackingRow] = await connection.execute(
                'SELECT order_id FROM returned_stock_tracking WHERE id = ? AND return_status = ?',
