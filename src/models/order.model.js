...
const updateRemittanceAmount = async (orderId, connection) => {
    // Utilise la connexion fournie (transaction) ou en crée une nouvelle
    const conn = connection || await dbConnection.getConnection();
    try {
        const [orderRows] = await conn.execute('SELECT status, payment_status, article_amount, delivery_fee, expedition_fee, amount_received FROM orders WHERE id = ?', [orderId]);
        ...
        if (order.status === 'delivered') {
            if (order.payment_status === 'cash') {
                remittanceAmount = parseFloat(order.article_amount) - parseFloat(order.delivery_fee) - parseFloat(order.expedition_fee);
            } else if (order.payment_status === 'paid_to_supplier') {
                remittanceAmount = -(parseFloat(order.delivery_fee) + parseFloat(order.expedition_fee));
            }
        } else if (order.status === 'failed_delivery') {
            remittanceAmount = parseFloat(order.amount_received) - parseFloat(order.delivery_fee) - parseFloat(order.expedition_fee);
        }

        await conn.execute('UPDATE orders SET remittance_amount = ? WHERE id = ?', [remittanceAmount, orderId]);
    } finally {
        // Libère la connexion uniquement si elle a été créée dans cette fonction
        if (!connection) {
            conn.release();
        }
    }
...
module.exports = {
    ...
    create: async (orderData) => {
        ...
        try {
            ...
            // Met à jour le montant à verser dès la création
            await updateRemittanceAmount(orderId, connection);
    ...
    update: async (orderId, orderData, userId) => {
        ...
        try {
            ...
            // Met à jour le montant à verser après modification
            await updateRemittanceAmount(orderId, connection);
    ...
    updateStatus: async (orderId, status, amountReceived = null, paymentStatus = null, userId) => {
        ...
        try {
            ...
            // Met à jour le montant à verser après un changement de statut
            await updateRemittanceAmount(orderId, connection);

            if (cashService && ((status === 'delivered' && paymentStatus === 'cash') || (status === 'failed_delivery' && amountReceived > 0))) {
                const [order] = await connection.execute('SELECT deliveryman_id, article_amount FROM orders WHERE id = ?', [orderId]);
                
                if (order[0] && order[0].deliveryman_id) {
                    const amountToRemit = (status === 'delivered') ? parseFloat(order[0].article_amount) : parseFloat(amountReceived);
                    const comment = `Versement en attente pour la commande n°${orderId}`;
                    await cashService.createRemittanceTransaction(order[0].deliveryman_id, amountToRemit, comment);
                }
            }
    ...
    updateRemittanceAmount // Exporter la nouvelle fonction
};