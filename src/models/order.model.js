...
const getBalanceImpactForStatus = (order, status, paymentStatus) => {
    ...
    if (status === 'delivered') {
        ...
        balanceImpact.revenue_articles = paymentStatus === 'cash' ? parseFloat(order.article_amount) : 0;
        balanceImpact.delivery_fees = parseFloat(order.delivery_fee);
        ...
        balanceImpact.delivery_fees = parseFloat(order.delivery_fee);
        // Pas de frais d'emballage pour une livraison ratée
    }
 ...
 * Cette fonction utilise une logique d'UPSERT pour créer ou mettre à jour le bilan.
...
const updateDailyBalance = async (connection, data) => {
    ...
    // Calcule l'impact sur le montant à verser
...
module.exports = {
    ...
    update: async (orderId, orderData, userId) => {
        ...
        try {
            ...
            const [oldOrderRows] = await connection.execute('SELECT * FROM orders WHERE id = ?', [orderId]);
            ...
            const newDate = moment(orderData.created_at).format('YYYY-MM-DD');

            // 1. Annuler l'impact de l'ancienne version sur le bilan
            ...
            // 3. Appliquer le nouvel impact sur le bilan
            const [newOrderRows] = await connection.execute('SELECT * FROM orders WHERE id = ?', [orderId]);
    ...
    updateStatus: async (orderId, newStatus, amountReceived = null, newPaymentStatus = null, userId) => {
        ...
        try {
            ...
            const [orderRows] = await connection.execute('SELECT * FROM shops s JOIN orders o ON s.id = o.shop_id WHERE o.id = ?', [orderId]);
            const order = orderRows[0];
            if (!order) throw new Error("Commande non trouvée.");

            const orderDate = moment(order.created_at).format('YYYY-MM-DD');

            // 1. Annuler l'impact de l'ancien statut
            ...
            // 2. Préparer et appliquer le nouvel impact
            let historyMessage = `Statut changé en ${newStatus}`;
            const updatedOrderData = { ...order, status: newStatus }; // Simuler les nouvelles données

            if (newStatus === 'delivered') {
                updatedOrderData.payment_status = newPaymentStatus;
                ...
                updatedOrderData.payment_status = 'pending'; // ou 'cancelled'
            }
            ...
            // 3. Mettre à jour la commande dans la BDD
            await connection.execute(
                'UPDATE orders SET status = ?, payment_status = ?, amount_received = ?, updated_by = ?, updated_at = NOW() WHERE id = ?',
                [newStatus, updatedOrderData.payment_status, amountReceived, userId, orderId]
            );
    ...
    remove: async (orderId) => {
        ...
        try {
            ...
            // 1. Annuler l'impact de la commande sur le bilan avant de la supprimer
            ...
            // 2. Supprimer la commande et ses dépendances
            await connection.execute('DELETE FROM order_items WHERE order_id = ?', [orderId]);
    ...
    // ... Autres fonctions (findAll, findById, assignDeliveryman, etc.) restent inchangées
