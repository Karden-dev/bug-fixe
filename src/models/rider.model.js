...
module.exports = {
    ...
    findRiderOrders: async (filters) => {
        ...
        try {
            // **MISE À JOUR : Ajout de o.is_urgent et de la sous-requête pour unread_count**
            let query = `
                SELECT 
                    o.*, 
                    s.name AS shop_name, 
                    u.name AS deliveryman_name,
                    o.is_urgent, -- Ajout du statut urgent
                    (
            ...
            if (filters.status && filters.status !== 'all') {
                // Gérer le cas où 'status' est un tableau (ex: ['pending', 'in_progress'])
    ...
    getOrdersCounts: async (riderId) => {
        const connection = await dbConnection.getConnection();
        try {
            // **MISE À JOUR :** Inclure tous les statuts possibles pour des compteurs complets
