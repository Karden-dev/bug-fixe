 ...
 * Récupère TOUTES les commandes éligibles d'un livreur pour une date,
 * et y joint les informations de versement si elles existent.
 */
const getOrdersForCashByDate = async (deliverymanId, date) => {
    const query = `
        ...
        WHERE
            ...
            AND o.payment_status = 'cash'
            AND (
                o.status = 'delivered' OR 
                (o.status = 'failed_delivery' AND o.amount_received IS NOT NULL AND o.amount_received > 0)
            )
        GROUP BY o.id
        ORDER BY o.id DESC
    `;
    const [rows] = await db.execute(query, [deliverymanId, date]);
    return rows;
};
    ...
    let query = `
        ...
        LEFT JOIN (
            ...
            WHERE o.deliveryman_id IS NOT NULL
                ...
                AND o.payment_status = 'cash'
                AND (o.status = 'delivered' OR (o.status = 'failed_delivery' AND o.amount_received > 0))
                AND NOT EXISTS (
...
/**
 * ## NOUVELLE FONCTION ##
 * Met à jour le montant d'une transaction de versement confirmée.
 */
...
const settleShortfall = async (shortfallId, amountPaid, userId, settlementDate) => {
    ...
    try {
        ...
        if (newStatus === 'partially_paid') {
            ...
            await connection.execute('UPDATE deliveryman_shortfalls SET amount = 0, status = "paid", settled_at = ? WHERE id = ?', [transactionDate, shortfallId]);
        }
