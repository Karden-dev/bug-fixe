...
const getAllDebts = async (req, res) => {
    try {
        const filters = {
            ...
            // NOUVEAU : Récupère les types à exclure (séparés par virgule, ex: 'daily_balance')
            excludeType: req.query.excludeType ? req.query.excludeType.split(',') : null
        };
...
const createDebt = async (req, res) => {
    try {
        const { shop_id, amount, type, comment, created_by, creation_date } = req.body; // Récupère la date de création
        if (!shop_id || !amount || !type || !created_by || !creation_date) {
            return res.status(400).json({ message: "Les champs shop_id, amount, type, created_by et creation_date sont requis." });
        }
        const newDebtId = await debtModel.create({ shop_id, amount, type, comment, created_by, created_at: creation_date });
