...
// --- NOUVELLE FONCTION AJOUTÉE POUR FCM ---

/**
 * Met à jour le token FCM du livreur actuellement authentifié.
 * (Utilisé par la RiderApp via la route PATCH /api/deliverymen/fcm-token)
 */
const updateFcmToken = async (req, res) => {
    // req.user est injecté par le middleware authenticateToken
    const userId = req.user.id; 
    // Le token est envoyé par la RiderApp
    const { fcm_token } = req.body;

    // Validation simple
    if (!fcm_token) {
        return res.status(400).json({ message: "Le 'fcm_token' est manquant dans le corps de la requête." });
    }

    // Le middleware isLivreur devrait déjà le vérifier, mais c'est une sécurité
    if (req.user.role !== 'livreur') {
         return res.status(403).json({ message: "Seuls les livreurs peuvent enregistrer un token FCM." });
    }
    ...
    try {
        ...
        console.error(`Échec de la mise à jour du token FCM pour l'utilisateur ${userId}:`, error);
        res.status(500).json({ message: "Erreur interne lors de la mise à jour du token." });
    }
};
