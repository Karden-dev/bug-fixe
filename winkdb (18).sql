...
--
-- Hôte : localhost:3306
-- Généré le : jeu. 06 nov. 2025 à 15:43
...
CREATE TABLE `orders` (
  ...
  `status` enum('pending','in_progress','ready_for_pickup','en_route','return_declared','delivered','cancelled','failed_delivery','reported','returned') NOT NULL DEFAULT 'pending',
...
CREATE TABLE `whatsapp_conversation_history` (
  ...
  `sender_type` enum('prospect_b2b','client_b2c','wink_agent_ai','wink_agent_humain') NOT NULL,
