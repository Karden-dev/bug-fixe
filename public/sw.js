...
// --- MISE À JOUR ICI ---
const STATIC_CACHE_NAME = 'wink-express-static-v8'; // Cache pour les fichiers de l'application (HTML, CSS, JS)
// --- FIN MISE À JOUR ---
...
const URLS_TO_CACHE = [
  ...
  '/dashboard.html',
  '/orders.html',
  '/deliverymen.html', // Assurez-vous que ce fichier existe si listé
  '/shops.html',
  '/reports.html',
  '/remittances.html',
  '/debts.html',
  ...
  '/rider-app.html',
  '/ridercash.html',
  // Potentiellement ajouter les nouvelles pages :
  ...
  '/js/deliverymenManager.js', // Ou le nom que vous avez choisi
  // Potentiellement ajouter le nouveau JS :
  ...
  'https://cdn.jsdelivr.net/npm/moment@2.29.1/moment.min.js',
   'https://cdn.jsdelivr.net/npm/chart.js', // Ajouter Chart.js si utilisé dans les nouvelles pages
  '/wink.png',
  '/wink-logo.png',
  '/favicon.ico',
...
self.addEventListener('install', event => {
  console.log('[Service Worker] Tentative d\'installation (v7)...'); // Log version
  // force l'installation immédiate d'une nouvelle version
  self.skipWaiting();
  event.waitUntil(
    caches.open(STATIC_CACHE_NAME).then(cache => {
      console.log('[Service Worker] Mise en cache des fichiers de l\'application (v7).');
      // Utiliser addAll avec { cache: 'reload' } force le re-téléchargement depuis le réseau
      // return cache.addAll(URLS_TO_CACHE.map(url => new Request(url, { cache: 'reload' })));
      // Ou plus simplement, addAll devrait déjà vérifier si les fichiers ont changé sur le serveur
      ...
      event.respondWith(
        caches.match(request).then(cachedResponse => {
          if (cachedResponse) {
            ...
            // console.log(`[Service Worker] Ressource servie depuis le cache statique: ${request.url}`);
            return cachedResponse;
          }
          // Si non trouvé dans le cache, aller sur le réseau
          // console.log(`[Service Worker] Ressource non trouvée en cache, fetch réseau: ${request.url}`);
          return fetch(request).then(networkResponse => {
              // Optionnel: Mettre en cache dynamique les ressources non pré-cachées ?
              // Attention: Cela peut remplir le cache rapidement. À utiliser avec précaution.
              // if (networkResponse.ok && !URLS_TO_CACHE.includes(url.pathname)) {
              ...
              // Optionnel: Renvoyer une page offline générique si nécessaire
              // return caches.match('/offline.html');
              // Ou juste laisser l'erreur se propager
          });
        })
      );
  } else {
      // Pour les requêtes cross-origin non API (CDN, etc.), utiliser la stratégie réseau par défaut
      // console.log(`[Service Worker] Requête cross-origin non API, utilise le réseau: ${request.url}`);
