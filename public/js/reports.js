...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const applyFiltersAndRender = () => {
        ...
        // Calculer les totaux sur TOUTES les données avant de filtrer
        updateGlobalTotals(allReports);

        // Filtrer les données pour l'affichage du tableau seulement
        filteredReports = allReports.filter(report => 
            (report.total_orders_sent > 0 || report.amount_to_remit !== 0) &&
            report.shop_name.toLowerCase().includes(searchTerm)
        );
        
        // Trier les résultats filtrés par nom
    ...
    if (processStorageBtn) {
        processStorageBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                // MISE À JOUR : GESTION DU MODE HORS LIGNE
                if (!navigator.onLine && window.syncManager) {
                    const request = {
                        url: `${API_BASE_URL}/reports/process-storage`,
                        method: 'POST',
                        payload: { date },
                        token: AuthManager.getToken() // Ajout du token pour l'authentification
                    };
                    await window.syncManager.put(request);
                    navigator.serviceWorker.ready.then(sw => sw.sync.register('sync-failed-requests'));
                    showNotification("Action mise en file d'attente. Elle sera exécutée dès que la connexion sera rétablie.", 'info');
                } else {
    ...
    if (confirmRecalculateBtn) {
        confirmRecalculateBtn.addEventListener('click', async () => {
            ...
            try {
                ...
                if (!navigator.onLine && window.syncManager) {
                    const request = {
                        url: `${API_BASE_URL}/reports/recalculate-report`,
                        method: 'POST',
                        payload: { date },
                        token: AuthManager.getToken() // Ajout du token
                    };
                    await window.syncManager.put(request);
                    navigator.serviceWorker.ready.then(sw => sw.sync.register('sync-failed-requests'));
                    showNotification("Action mise en file d'attente. Elle sera exécutée dès que la connexion sera rétablie.", 'info');
    ...
    const initializePage = () => {
        ...
        logoutBtn?.addEventListener('click', () => { 
            // Utilise le AuthManager pour la déconnexion
            AuthManager.logout();
        });
