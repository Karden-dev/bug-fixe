    ...
    reportsTableBody?.addEventListener('click', async (e) => {
        ...
        if (button) {
            ...
            try {
                ...
                const deliveryFees = parseFloat(reportDetails.total_delivery_fees || 0);
                if (deliveryFees > 0) {
                    reportContent += `*Total Frais de livraison :* ${formatAmount(deliveryFees)}\n`;
                }
                
                const expeditionFees = parseFloat(reportDetails.total_expedition_fees || 0);
                if (expeditionFees > 0) {
                    reportContent += `*Total Frais d'expédition :* ${formatAmount(expeditionFees)}\n`;
                }

                const packagingFees = parseFloat(reportDetails.total_packaging_fees || 0);
                if (packagingFees > 0) {
                    reportContent += `*Total Frais d'emballage :* ${formatAmount(packagingFees)}\n`;
                }

                const storageFees = parseFloat(reportDetails.total_storage_fees || 0);
                if (storageFees > 0) {
                    reportContent += `*Total Frais de stockage (jour) :* ${formatAmount(storageFees)}\n`;
                }

                const previousDebts = parseFloat(reportDetails.previous_debts || 0);
                if (previousDebts > 0) {
                    reportContent += `*Créances antérieures :* ${formatAmount(previousDebts)}\n`;
                }
