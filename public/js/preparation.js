...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const renderOrders = (orders) => {
    ...
    sortedGroupIds.forEach(deliverymanId => {
      ...
      groupHeader.className = 'deliveryman-header p-2 bg-light border rounded mb-2';
      groupHeader.innerHTML = `<h5 class="mb-0"><i class="bi bi-person-fill me-2"></i>${group.name}
             <span class="badge text-bg-info ms-2">${readyCount} Prêt(s)</span>
             <span class="badge text-bg-warning ms-2">${inProgressCount} En cours</span></h5>`;
