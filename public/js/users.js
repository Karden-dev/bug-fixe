...
/**
 * Formate une chaîne de date en format jj/Mois/aaaa.
...
const handleUserFormSubmit = async (e) => {
    ...
    try {
        ...
        addUserModal.hide();
        fetchAndRenderUsers();
    } catch (error) { alert(error.response?.data?.message || "Erreur lors de l'enregistrement."); }
};

/**
 * Gère le clic sur les actions du tableau (édition, changement de statut).
...
/**
 * Gère la soumission du formulaire de changement de PIN.
 * @param {Event} e - L'événement de soumission.
 */
const handleChangePinSubmit = async (e) => {
    ...
    try {
        ...
        changePinModal.hide();
        addUserModal.hide();
    } catch (error) { alert(error.response?.data?.message || "Erreur lors du changement de PIN."); }
};

/**
 * Gère la réinitialisation de la modale à sa fermeture.
 */
...
const initializeApp = () => {
    ...
    document.getElementById('addUserModal').addEventListener('hidden.bs.modal', handleModalHidden);

    // Peupler le select des rôles (selon la logique trouvée dans l'original)
