...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Le middleware attend 'Authorization: Bearer VOTRE_TOKEN'.
    if (user.token) {
    ...
    const editRemittanceOrderModal = new bootstrap.Modal(document.getElementById('editRemittanceOrderModal')); 
    ...
    const editRemittanceOrderForm = document.getElementById('editRemittanceOrderForm'); 
            ...
            const actionsHtml = `
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu">
                        ...
                        <li><a class="dropdown-item edit-remittance-btn" href="#" data-tx-id="${order.remittance_transaction_id}" data-order-id="${order.order_id}" data-amount="${order.remittance_amount_tx || order.expected_amount}"><i class="bi bi-pencil me-2"></i>Modifier</a></li>
                    </ul>
                </div>
            `;
    ...
    const handleAmountConfirmationSubmit = async (e) => {
        ...
        if (isNaN(paidAmount) || paidAmount < 0) {
            amountError.textContent = "Veuillez entrer un montant valide.";
    ...
    const handleEditRemittance = (target) => {
        const txId = target.dataset.txId;
        const orderId = target.dataset.orderId;
        const currentAmount = target.dataset.amount;

        if (!txId) {
            showNotification("Cette action n'est possible que pour un versement déjà confirmé.", "warning");
            return;
        }

        remittanceIdToEdit = txId;

        document.getElementById('editRemittanceOrderId').textContent = `Modification du versement pour la Commande #${orderId}`;
        document.getElementById('editRemittanceAmountInput').value = parseFloat(currentAmount).toFixed(0);

        editRemittanceOrderModal.show();
    };

    const handleEditRemittanceSubmit = async (e) => {
        e.preventDefault();
        const amount = document.getElementById('editRemittanceAmountInput').value;
        
        if (!remittanceIdToEdit) return;
        
        try {
            await axios.put(`${API_BASE_URL}/cash/remittances/${remittanceIdToEdit}`, { amount });
            showNotification(`Montant du versement #${remittanceIdToEdit} mis à jour.`);
            
            editRemittanceOrderModal.hide(); 
            remittanceDetailsModal.hide();
            
            applyFiltersAndRender();
        } catch (error) { 
            showNotification(error.response?.data?.message || "Erreur lors de la modification du montant.", 'danger'); 
        }
        remittanceIdToEdit = null;
    };
    ...
    const initializeEventListeners = () => {
        ...
        if (editRemittanceOrderForm) editRemittanceOrderForm.addEventListener('submit', handleEditRemittanceSubmit); 
        ...
        document.body.addEventListener('click', (e) => {
            ...
            const actions = {
                ...
                '.settle-shortfall-btn': () => handleSettleShortfall(target),
                '.edit-shortfall-btn': () => handleEditShortfall(target),
                '.delete-shortfall-btn': () => handleDeleteShortfall(target),
                ...
                '.edit-remittance-btn': () => handleEditRemittance(target), 
            };
