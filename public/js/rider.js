...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    /**
     * Enregistre une requête pour une synchronisation en arrière-plan.
     * @param {string} url - L'URL de l'API à appeler.
     * @param {string} method - La méthode HTTP (POST, PUT, DELETE).
     * @param {object} payload - Les données à envoyer.
     */
    async function queueFailedRequest(url, method, payload) {
        ...
        if ('serviceWorker' in navigator && 'SyncManager' in window) {
            try {
                ...
                console.log('WinkDev PWA: Synchronisation en arrière-plan enregistrée.');
            } catch (err) {
                console.error('WinkDev PWA: Impossible d\'enregistrer la synchronisation.', err);
            }
        }
    }
    ...
    const handleAuthError = (error) => {
        if (error.response?.status === 401 || error.response?.status === 403) {
            ...
            // En mode hors ligne, une erreur réseau est normale, on ne déconnecte pas.
            showNotification("Vous êtes hors ligne. Certaines données peuvent ne pas être à jour.", "warning");
        } else {
            console.error(error);
                    ...
                    <div class="order-actions">
                        ...
                        <div class="dropdown">
                            ...
                            <ul class="dropdown-menu">
                                ...
                                <li><a class="dropdown-item" href="https://wa.me/${customerPhone.replace(/\s/g, '')}"><i class="bi bi-whatsapp"></i> WhatsApp</a></li>
                            </ul>
                        </div>
                    </div>
                </div>
    ...
    const fetchOrders = async (tabName, searchQuery = '') => {
        ...
        try {
            ...
            ordersContainer.innerHTML = `<p class="text-center text-danger mt-5">Erreur de chargement. Vérifiez votre connexion.</p>`;
            handleAuthError(error);
        }
    };
    ...
    document.getElementById('confirmReturnBtn').addEventListener('click', async () => {
        ...
        try {
            if (!navigator.onLine) {
                showNotification('Mode hors ligne. La déclaration sera synchronisée plus tard.', 'info');
                await queueFailedRequest(`${API_BASE_URL}/orders/${currentOrderId}/status`, 'PUT', payload);
                returnModal.hide();
                return;
            }
            await axios.put(`${API_BASE_URL}/orders/${currentOrderId}/status`, payload);
            showNotification('Déclaration de retour enregistrée.');
            fetchOrders('returns');
        } catch (error) {
             if (!navigator.onLine) {
                showNotification('Mode hors ligne. La déclaration sera synchronisée plus tard.', 'info');
    ...
    // La gestion du formulaire de versement est dans ridercash.js, on la laisse ici commentée au cas où
    ...
    const updateOrderStatus = async (orderId, status, paymentStatus = null, amountReceived = 0) => {
        ...
        try {
            if (!navigator.onLine) {
                showNotification('Mode hors ligne. Le statut sera synchronisé plus tard.', 'info');
                await queueFailedRequest(`${API_BASE_URL}/orders/${orderId}/status`, 'PUT', payload);
                // Optionnel : Mettre à jour l'UI localement pour un retour visuel immédiat
                return;
            }
