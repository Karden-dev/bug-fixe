...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const renderOrdersTable = (orders) => {
        ...
        orders.forEach(order => {
            ...
            const displayedItemName = order.items && order.items.length > 0 
                ? truncateText(order.items[0].item_name, 20) : 'N/A';
            ...
            row.innerHTML = `
                ...
                <td><span data-bs-toggle="tooltip" data-bs-html="true" title="${itemTooltip}">${displayedItemName}</span></td>
    ...
    const setupDeliverymanSearch = () => {
        deliverymanSearchInput.addEventListener('input', () => {
            ...
            if (searchTerm.length > 1) {
                // Utilise le cache des livreurs actifs
