...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Cartes de statistiques globales (IDs vérifiés selon deliverymen.html)
    ...
    // Note: L'élément pour le CA total n'existe pas dans les cartes du HTML fourni.
    ...
    const settingUserStatusSelect = document.getElementById('settingUserStatus'); // Ajout pour le statut
    ...
    // Modale Ajout Absence (depuis modale détails)
    ...
    // Modale Gestion Absences/Fériés (Globale)
    ...
    const absenceListPeriodEl = document.getElementById('absenceListPeriod'); // Pour afficher la période

    // Modales Actions Groupées (Instances Bootstrap - Vérifier existence)
    ...
    const fetchAndRenderMainData = async () => {
        ...
        try {
            ...
            // !! L'API /api/deliverymen/performance DOIT être mise à jour !!
            // Elle doit renvoyer les livreurs triés par performance (ex: delivered_orders DESC)
            // et inclure les champs: id, name, received_orders, in_progress_orders, cancelled_orders, delivered_orders, total_revenue
            const [statsRes, perfRes] = await Promise.all([
                axios.get(`${API_BASE_URL}/deliverymen/stats`, { params, headers }),
                axios.get(`${API_BASE_URL}/deliverymen/performance`, { params, headers }) // ATTENTION: API à mettre à jour pour le tri et les champs
            ]);

            updateStatsCards(statsRes.data);
            renderDeliverymenTable(perfRes.data); // Passe les données de performance triées
    ...
    const updateStatsCards = (stats = {}) => {
        if (totalDeliverymenEl) totalDeliverymenEl.textContent = stats.total ?? '--'; // Utilise total pour Total Actifs selon HTML
        if (workingDeliverymenEl) workingDeliverymenEl.textContent = stats.working ?? '--'; // Présents/Actifs
    ...
    const renderDeliverymenTable = (livreurs) => {
        ...
        // Assurez-vous que les données sont triées si l'API ne le fait pas (optionnel)
        // livreurs.sort((a, b) => (b.delivered_orders || 0) - (a.delivered_orders || 0));
        ...
        livreurs.forEach((livreur, index) => {
            ...
            // **Logique de Classement Visuel Réintroduite**
            let distinctionHtml = '';
    ...
    // (renderModalPerformance, updateModalRemuneration, updateModalAdminObjectives, renderModalChart - identiques à la version précédente)
    const renderModalPerformance = (data) => {
        ...
        modalPerformanceContentEl.innerHTML = `
            <div class="row g-3 mb-3">
                 <div class="col-md-3"><div class="card card-body text-center shadow-sm"><div class="stat-label">Reçues</div><div class="stat-value">${stats.received ?? '--'}</div></div></div>
                 <div class="col-md-3"><div class="card card-body text-center shadow-sm"><div class="stat-label">Livrées*</div><div class="stat-value text-success">${stats.delivered ?? '--'}</div></div></div>
                 <div class="col-md-3"><div class="card card-body text-center shadow-sm"><div class="stat-label">Taux Livr.</div><div class="stat-value text-info">${rate}%</div></div></div>
                 <div class="col-md-3"><div class="card card-body text-center shadow-sm"><div class="stat-label">Jours Actifs</div><div class="stat-value">${stats.workedDays ?? '--'}</div></div></div>
            </div>
    ...
    const updateModalRemuneration = (data) => {
     ...
     const updateModalAdminObjectives = (data) => {
         ...
         // Assurer que achieved et target sont des nombres avant division
         ...
         container.innerHTML = `
             ...
             <div class="progress mt-2 mb-2" style="height: 1.25rem;" title="${achieved} / ${target > 0 ? target : '--'} courses">
                 <div class="progress-bar ${progressClass}" role="progressbar" style="width: ${percentage.toFixed(1)}%;"
    ...
    /**
     * NOUVELLE FONCTION: Récupère et affiche le journal de performance.
     */
    ...
    const fetchAndRenderAbsences = async (userId) => {
        ...
        try {
            // !! API À CRÉER : GET /api/deliverymen/:userId/absences !!
            // Ou utiliser GET /api/schedule/absences?userId=...
    ...
    const initializeEventListeners = () => {
        ...
        livreurSettingsForm?.addEventListener('submit', async (e) => {
            ...
            try {
                 // !! API À CRÉER : PUT /api/deliverymen/:userId/settings !!
                 // Cette API doit mettre à jour ET la table 'livreurs' ET le statut dans la table 'users'
        ...
        absencesModalEl?.addEventListener('show.bs.modal', async () => {
             ...
             try {
                 ...
                 // Laisser activé pour permettre sélection multiple même si type='absence' par défaut
                 manageAbsenceLivreurSelect.disabled = (manageAbsenceTypeSelect.value === 'ferie'); // Correction: utiliser la valeur actuelle
                 await fetchGlobalAbsencesList();
             } catch (error) {
