// lib/screens/orders_returns_screen.dart
...
class _OrdersReturnsScreenState extends State<OrdersReturnsScreen> {
  ...
  void initState() {
    super.initState();
    // Définit la plage de dates par défaut (30 derniers jours) et lance le fetch
    _setInitialDatesAndFetch();
  }

  void _setInitialDatesAndFetch() {
    final now = DateTime.now();
    // Défaut: 30 derniers jours pour les retours
    _startDate = now.subtract(const Duration(days: 29)).copyWith(hour: 0, minute: 0, second: 0, millisecond: 0);
  ...
  Future<void> _fetchOrders() async {
    ...
    setState(() {
      _ordersFuture = orderService.fetchRiderOrders(
        // Statuts pour les retours
        statuses: ['return_declared', 'returned', 'return_pending'], // Inclut aussi 'return_pending' si applicable
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Retours Déclarés'),
      ),
      body: Column(
        children: [
              ...
              builder: (context, snapshot) {
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    // Réutilise OrderCard. La bordure rouge sera appliquée car le statut est return_declared ou returned.
