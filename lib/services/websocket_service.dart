...
// import '../models/order.dart'; // Importation supprimée car inutilisée
...
class WebSocketService extends ChangeNotifier {
  ...
  void _handleInboundEvents(String type, Map<String, dynamic>? payload) {
    ...
    switch (type) {
      case 'NEW_ORDER_ASSIGNED':
        ...
        // Utilise ?? '' pour éviter les erreurs si les clés sont manquantes
      ...
      case 'NEW_MESSAGE':
        ...
        // S'assurer que le contenu du message est bien une chaîne
        body = payload?['message_content'] as String? ?? 'Nouveau message reçu.';
  ...
  Future<List<Map<String, dynamic>>> fetchMessages(int orderId, {int? lastMessageId}) async {
    try {
      ...
      // S'assurer que la réponse est bien une liste
      if (response.data is List) {
         return List<Map<String, dynamic>>.from(response.data);
      } else {
         debugPrint('WS: fetchMessages a reçu une réponse non-List: ${response.data}');
         return []; // Retourner une liste vide en cas de format inattendu
      }
    } on DioException catch (e) {
