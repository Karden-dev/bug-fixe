...
// CORRIGÉ : Utilisation du chemin package
...
class OrderService {
  ...
  Future<List<Order>> fetchRiderOrders({
    ...
    try {
      // CORRECTION DU BUG: Passer la liste [statuses] directement à Dio.
      // Dio gère l'encodage correct en ?status=a&status=b pour les backends Express.
      final response = await _dio.get(
        '$_apiBaseUrl/orders',
        queryParameters: {
          'status': statuses, // MODIFIÉ: Passer la LISTE directement.
          'startDate': startDate,
          'endDate': endDate,
          'search': search,
  ...
  Future<void> updateOrderStatus({
    ...
    final String url = "https://app.winkexpress.online/api/orders/$orderId/status";
  ...
  Future<void> declareReturn(int orderId) async {
    ...
    final String url = "https://app.winkexpress.online/api/orders/$orderId/declare-return";
